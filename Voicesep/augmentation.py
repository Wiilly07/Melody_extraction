...
def reorder(song):
    song.df_multitrack.sort_values(by=['start', 'note', 'duration', 'label'], ascending=[True, True, False, False], inplace=True)
...
def onoff_shift_random(song):
    if not np.random.choice(3): #1/3 chance
...
def chop_note(self, notes, segs):
    newdata = []
    for note, seg in zip(notes, segs):
        note_list = song.df_multitrack.loc[note].to_list()
        new_duration = note_list[2] * (480/479) / seg    # (480/479)可能有需要改進
        newdata = newdata + [[note_list[0], note_list[1]+new_duration*i, note_list[2]/seg, note_list[3]] for i in range(seg)]
    
    Max_id = len(song)
    for i in range(len(newdata)):
        self.df_multitrack.loc[Max_id+i] = newdata[i]
        
    song.df_multitrack = song.df_multitrack.astype({'note':'int64', 'label':'int64'})

    song.df_multitrack.drop(notes, inplace=True)
    song.df_multitrack.sort_values(by=['start', 'note'], inplace=True)
    song.df_multitrack.reset_index(drop=True, inplace=True)

class Augmenter:
    def __init__(self, aug_ls, inplace=True):
